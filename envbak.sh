...
for config in "${configs[@]}"
    do  
        if ! grep -q "^$config$" ./store.conf ; then #If config is not already in store.conf
           N=$(cat ./store.conf | grep -n $1 | cut -f 1 -d :) #Find line with utility name.
...
delete()
{
	if [ -f "./store.conf" ]; then #If config file exist 
		if $hasConfig ; then #If command have setted exact config files to remove.
			for config in "${configs[@]}"
			do
				sed -i "$(cat ./store.conf | grep -n "^$config$" | cut -f 1 -d :)d" ./store.conf #Remove exact configs
			done
			...
			if  grep -q "^Util:$1:" ./store.conf ; then #If record about utility is in store.conf
				startOfSec=$(cat ./store.conf | grep -n "Util:$1:"| cut -f 1 -d :) #Line where is start of record (Util:[NAME]:<PATH>)
	...
	if [ -f "./store.conf" ] || $2 ; then
		...
		do
			if $( echo $line | grep -q "^Util:") ; then #If line is start of util record 
				...
				if [ ! -z $installScript ] && [ -f $installScript ]; then #If path to installScript.sh is not default(empty) and exists.
